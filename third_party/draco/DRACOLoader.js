...
DRACOLoader.loadWebAssemblyDecoder = function(dracoDecoder) {
  DRACOLoader.dracoDecoderType['wasmBinaryFile'] =
...
DRACOLoader.loadDracoDecoder = function(dracoDecoder) {
  if (
    ...
    DRACOLoader.dracoDecoderType.type === 'js'
  ) {
    // No WebAssembly support
    DRACOLoader.loadJavaScriptFile(
      dracoDecoder.dracoSrcPath + 'draco_decoder.js',
      null,
      dracoDecoder
    );
  } else {
    DRACOLoader.loadJavaScriptFile(
      dracoDecoder.dracoSrcPath + 'draco_wasm_wrapper.js',
